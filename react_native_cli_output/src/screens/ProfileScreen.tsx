...
export function ProfileScreen() {
  ...
  const handlePickerResponse = async (result: ImagePickerResponse) => {
    if (result.didCancel) return;
    if (result.errorCode) {
      Alert.alert('Image selection failed', result.errorMessage || 'Unable to access the selected image.');
      return;
    }

    const asset = result.assets?.[0];
    if (!asset?.uri) {
      Alert.alert('Image selection failed', 'No usable image was returned.');
      return;
    }
    ...
    Alert.alert('Update profile photo', 'Choose an image source.', [
      {
        ...
        onPress: () => void launchCamera(
          ...
          (result) => void handlePickerResponse(result),
        ),
      },
      {
        text: 'Photo library',
        onPress: () => void launchImageLibrary(
          {
            mediaType: 'photo',
            selectionLimit: 1,
            quality: 0.8,
